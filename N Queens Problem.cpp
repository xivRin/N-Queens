...
std::vector<coordinate> NQueens(int n) {
    int filled = 0;
    ...
    while (filled < n) {
        ...
        if (!safe && (stack.top() < n)) {
            ...
            if (filled == -1) {
                ...
                std::vector<struct coordinate> returnVector;
                returnVector.push_back(coordinate(0, 0));
                return returnVector;
            }
            bool isEmpty = queens.empty();
            queens.pop_back();
            stack.pop();
            filled = filled--;
            int nextColumn = stack.top() + 1;
...
int main() {
    int n = getn();
    std::vector<coordinate> ans = NQueens(n);
    for (int i = 0; i < n; i++) {
        coordinate temp = ans[i];
